...
const User = ({ user, currentUseId }: IUserProps) => {
 
  const messagesWithCurrentUser = user.conversations.find(
    (conversation: TConversation) => 
      conversation.users.find((user) => user.id === currentUseId)
  );

  const latestMessage = messagesWithCurrentUser?.messages.slice(-1)[0];

  return (
    <div className='grid grid-cols-[40px_1fr_50px] grid-rows-[40px] gap-3 px-4
      border-b-[1px] hover:cursor-pointer hover:bg-orange-500'>
        ...
        <div>
          ...
          {latestMessage && <p className='overflow-hidden text-xs font-medium text-gray-600
